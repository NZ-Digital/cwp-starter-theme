<div class="latestNewsBanner-container">
    <div class="row no-gutters">
        ...
        <div class="col-lg-7<% if $ContentPosition == 'cp-left' %> order-first<% end_if %>">
            <div class="image-content--content $ImageHeight"<% if $ContentBg %> style="background-image: url('{$ContentBg.URL}'); <% if $ContentBgColor %>background-color:#{$ContentBgColor};<% end_if %>" <% end_if %>>
                ...
                <% if $News.Page %>
                    <div class="content-body"><p class="fontsize16 font-weight-medium"><a href="$News.Page.Link" class="d-flex"><span class="theme-button-yellow-small-arrow">Read More</span></a></p></div>
                <% end_if %>
            </div>
        </div>
    </div>
